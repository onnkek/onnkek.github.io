window.addEventListener("DOMContentLoaded", () => {
  ...
  function MoveNode(cursor) {
    ...
    if(selectEditBranch) {
      ...
      let newPoint = { "x": selectEditBranch.coordinates.x += cursor.x - point.x, "y": selectEditBranch.coordinates.y += cursor.y - point.y};
    ...
    if (select && point) {
      ...
      for (let i = 0; i < branches2.length; i++) {
        if (branches2[i].image.pole2.nodePole) {
          let points = "";
          branches2[i].image.list[
            branches2[i].image.list.length - 1
          ].coordinates.x = branches2[i].image.pole2.nodePole.coordinates.x;
          branches2[i].image.list[
            branches2[i].image.list.length - 1
          ].coordinates.y = branches2[i].image.pole2.nodePole.coordinates.y;
          ...
          branches2[i].image.pole2.circle.setAttribute(
            "cx",
            branches2[i].image.pole2.nodePole.coordinates.x
          );
          branches2[i].image.pole2.circle.setAttribute(
            "cy",
            branches2[i].image.pole2.nodePole.coordinates.y
          );
      ...
      let nodeLogic = scheme.nodes.filter((x) =>
        x.image.cp.find(
          (cp) =>
            Math.abs(cp.coordinates.x - selectBranchCP.coordinates.x) <
              hitRadiusNode &&
            Math.abs(cp.coordinates.y - selectBranchCP.coordinates.y) <
              hitRadiusNode
        )
      );
          ...
          for (let i = 0; i < scheme.nodes[j].image.cp.length; i++) {
            ...
            if (scheme.nodes[j].animateOut < 1) {
              animate({
                ...
                timing: funcOut1,
                draw: function (progress) {
                  scheme.nodes[j].image.cp[i].circle.setAttribute(
                    "r",
                    progress
                  );
                },
      ...
      let node = scheme.nodes.find((x) =>
        x.image.cp.find(
          (cp) =>
            Math.abs(cp.coordinates.x - selectBranchCP.coordinates.x) < 20 &&
            Math.abs(cp.coordinates.y - selectBranchCP.coordinates.y) < 20
        )
      );
      if (node) {
        selectNodeCP = node.image.cp.find(
          (cp) =>
            Math.abs(cp.coordinates.x - selectBranchCP.coordinates.x) < 20 &&
            Math.abs(cp.coordinates.y - selectBranchCP.coordinates.y) < 20
        );
  ...
  scheme.branches.map((x) => {
    x.image.pole1.circle.addEventListener("mousedown", (e) => {
      selectBranchCP = scheme.branches.find(
        (x) => x.image.pole1.circle == e.target
      ).image.pole1;
    });
    x.image.pole2.circle.addEventListener("mousedown", (e) => {
      selectBranchCP = scheme.branches.find(
        (x) => x.image.pole2.circle == e.target
      ).image.pole2;
    });
  });
  ...
  window.addEventListener("mousedown", (e) => {
    ...
    if (!test && selectBranch && e.shiftKey) {
      ...
      for (let i = 0; i < selectBranch.image.list.length - 1; i++) {
        ...
        let len =
          Math.abs(
            (y2 - y1) * e.clientX - (x2 - x1) * e.clientY + x2 * y1 - y2 * x1
          ) / Math.sqrt(Math.pow(y2 - y1, 2) + Math.pow(x2 - x1, 2));

        let da = Math.sqrt(
          Math.pow(x1 - e.clientX, 2) + Math.pow(y1 - e.clientY, 2)
        );
        let db = Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
        let dc = Math.sqrt(
          Math.pow(x2 - e.clientX, 2) + Math.pow(y2 - e.clientY, 2)
        );
